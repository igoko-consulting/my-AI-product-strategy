...
6. **The flat base fee is regressive against trip value, and probably leaves money on the table.** At $6.49 a $2,500 trip pays 0.26% and an $8,000 trip pays 0.08%, so the fee shrinks exactly as the stakes rise. Travel insurance runs 4–8% of trip cost, which is an order of magnitude of headroom. The candidate fix is to tier the base and hold the metered fee flat — $4.99 under $1,500, $6.49 from $1,500 to $4,000, $12.99 above $4,000, with $35.00 per resolution in every band. **Price the watching by what is at stake; price the work by what it costs to do**, since finding a seat costs the same whatever the trip cost. Untested, confidence ~70%.
   ...
   **Challenged by the proxy interviews.** The premium participant's objection was not the price but the shape: a per-incident fee "reads as a mass-market structure" at that tier, and the preference was for it bundled into an existing premium account or subscription, invisible at the point of use. If that holds with real premium travellers, the fix is not a larger fee in the same shape — it is a different SKU. Note the source: a role-played session, not a target-user interview.
...
8. **The metered fee has no defined trigger event.** The model charges $35.00 "per disruption resolved" without stating what resolved means. If a traveller books their own replacement while our hold is live, we have done the detection, the monitoring and the holding — all the cost — and none of the booking. Charging looks like billing someone for their own work; not charging runs the expensive part on cases that never bill, sharpening the 100%-of-trips against 3.6%-of-revenue asymmetry the base fee already strains against. There is also a perverse incentive: if the fee only fires when we book the fix, there is quiet pressure to make self-rescue slower or less visible. Golden dataset row 12 in M4 answers it one way — release the hold, record it as resolved-by-traveller, do not bill — but that is a product decision taken inside a test case, and the pricing model has never stated it. Raised independently by two sets of proxy-interview participants.

9. **Billing $35 at the moment of disruption charges the customer on the worst day of their trip.** Worth testing a variant where the first resolution is included in the base and only subsequent ones are metered. The revenue difference is small; the goodwill difference may not be.
